...
#import dev, test, train (first row is column names)
dev_data = datasets.load_dataset("csv", data_files=location + "dev_atomic.tsv", delimiter="\t")
dev_data = dev_data.rename_column("label", "labels")
train_data = datasets.load_dataset("csv", data_files=location + "train_atomic.tsv", delimiter="\t")
...
num_added_toks = tokenizer.add_special_tokens({
	...
	#"additional_special_tokens": [PREMISE_SPECIAL, HYPOTHESIS_SPECIAL]
})
...
output_path = "sloberta_train_final_with_atomic"
